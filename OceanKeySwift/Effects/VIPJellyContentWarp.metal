...
[[ stitchable ]] float2 vipJellyContentWarp(
    ...
    float unitX = clamp(position.x / width, 0.0, 1.0);
    float unitY = clamp(position.y / height, 0.0, 1.0);
    ...
    float vertical = (slow * 0.54 + medium * 0.30 + fast * 0.10) * amplitude * falloff;

    float sideWave = sin((unitY * 2.4 + t * 0.37 + seed * 11.0) * 6.2831853);
    float horizontal = sideWave * amplitude * 0.18 * (0.45 + edgeBias * 0.55);
